...
- On se connecte en tant que level8:
```
...
>Si l'entrée utilisateur ne commence pas par "auth ", on `jmp` en `<main+222>`.
...
>Si notre entrée ne commence pas par "service ", on `jmp` en `<main+337>`.
...
>Si notre entrée ne commence pas par "login ", on `jmp` en `<main+16>` pour une nouvelle boucle.
...
> Si `0x8049aac + 0x20` n'est pas égal à 0, on ouvre un shell.

- Pour résumé, le programme peut prendre différentes entrées et les compare à des chaînes de caractères. Si les entrées correspondent à certaines chaînes, il effectue des actions spécifiques.

- Si on écrit "login" et que la valeur pointée par `0x8049aac + 0x20` est différente de 0, on ouvre un shell. Il est donc nécessaire que `0x8049aac` contienne une adresse, et que la valeur à `cette_adresse + 0x20` soit différente de 0.
